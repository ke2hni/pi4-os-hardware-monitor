...
APP_NAME="Pi 4 OS Hardware Monitor"
...
if ! is_pi4; then
    fail "This app is for Raspberry Pi 4 hardware only. Detected: $(tr -d '\0' </proc/device-tree/model 2>/dev/null || echo 'Unknown')"
fi
...
Type=Application
Name=${APP_NAME}
Comment=Raspberry Pi 4 desktop hardware monitor
Exec=${BIN_PATH}
Icon=${APP_ID}
Terminal=false
...
validate_install() {
    ...
    if [[ -n "${SUDO_USER_NAME}" ]] && id "${SUDO_USER_NAME}" >/dev/null 2>&1; then
        log "User ${SUDO_USER_NAME} was added to ${REQUIRED_GROUP}. Log out/in if sudo-free storage SMART access does not work immediately."
    fi
}
